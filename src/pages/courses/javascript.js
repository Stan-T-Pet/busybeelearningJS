// File: src/pages/courses/javascript.js
...
export async function getServerSideProps(context) {
  try {
    await connectDB();
    const lessons = await Lesson.find({ subject: "javascript" }).lean();
    const lessonsJSON = lessons.map((lesson) => JSON.parse(JSON.stringify(lesson)));
    return { props: { lessons: lessonsJSON } };
  } catch (error) {
    console.error("Error fetching javascript lessons:", error);
    return { props: { lessons: [] } };
  }
}

export default function javascriptCourse({ lessons }) {
  return (
    <Box>
      <Header />
      <Container sx={{ mt: 4 }}>
